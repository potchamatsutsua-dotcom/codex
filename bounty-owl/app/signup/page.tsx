...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (password.length < 8) {
      toast({ title: "エラー", description: "パスワードは8文字以上で入力してください", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "確認メールを送信しました", description: "メールのリンクをクリックして登録を完了してください" });
      router.push("/login");
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-background to-muted/30 p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold">無料アカウント作成</h1>
          <p className="text-muted-foreground mt-1">30秒で始められます</p>
        </div>
